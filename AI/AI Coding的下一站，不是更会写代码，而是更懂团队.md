...
> 公众号：腾讯程序员
> 原文链接：[https://mp.weixin.qq.com/s/kz_7d66dhmPhOUlcMh4MzA](https://mp.weixin.qq.com/s/kz_7d66dhmPhOUlcMh4MzA)
...
![](./assets/AI Coding的下一站，不是更会写代码，而是更懂团队/745c436d20c739f2375c17408d109257.png)
...
![](./assets/AI Coding的下一站，不是更会写代码，而是更懂团队/7dc90dcd1d6d9b96cba784d04d7451da.png)
...
值得注意的是，这 260 条经验是分 4 批独立进行的，batch 之间表现差异明显。最极端的是 batch\_004，仅识别出 6 个人工正例中的 1 个——该 batch 的正例全部落在非"双强一致"的组合上。这说明去重 prompt 对边界的敏感度在输入特征分布变化时仍有波动。
...
**Update 必须自检**。 对每一笔 update 增加内部 quality\_check：合并后的 When 是否过度泛化（丢失了原经验的场景约束）、What 是否保留了最具体、最安全、最可执行的操作建议、Why 是否保留了核心机制、是否引入了不受原始经验支持的新结论。Update 过判是当前最突出的问题—— Recall 高达 96.30% 但 Precision 只有 78.79%。也就是说大部分该 update 的都被识别了，但模型存在明显的"积极合并"倾向，把一些本该 create 的内容也判成了 update。
...
![](./assets/AI Coding的下一站，不是更会写代码，而是更懂团队/efa83ed23f2bf71ac71c6635f7066de5.png)
...
![](./assets/AI Coding的下一站，不是更会写代码，而是更懂团队/c7fb6ac1809dd7d4b411df8281368e08.png)
...
![](./assets/AI Coding的下一站，不是更会写代码，而是更懂团队/4a3a0689e0d56493eec35c5111aa6cc6.png)

在后续与 Agent 协作的开发场景里，我们通过 TDev 驱动需求实现的流程时，对话内部在特定阶段（探索）自动触发对历史经验的召回尝试：

![](./assets/AI Coding的下一站，不是更会写代码，而是更懂团队/7b52861b7e4dcf6400d292decee2142d.png)
...
在最新 125 次真实开发检索的召回表现：请求级召回率68.8%，no\_hit（未命中）仅 4.8%，平均每次注入 **2.4** 条经验，平均检索耗时 **1,299 ms**。
