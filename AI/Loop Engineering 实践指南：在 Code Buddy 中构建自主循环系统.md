...
> 公众号：腾讯程序员
> 原文链接：[https://mp.weixin.qq.com/s/YqIyL7uW4EV2r5HLDW7wcA](https://mp.weixin.qq.com/s/YqIyL7uW4EV2r5HLDW7wcA)
...
1. **Discover**：扫描 `auth/legacy` 目录，识别所有导出和调用点
2. **Plan**：列出需要修改的文件，制定迁移步骤
3. **Execute**：逐个文件修改导入路径，更新调用方式
4. **Verify**：运行 `npm test` 和 `npm run build`，检查编译和测试结果
...
4. **兜底上限**：始终加 `or stop after N turns`，防止无限循环消耗 token
5. **条件不超过 4000 字符**：这是 `/goal` 的硬性限制
